...
const DishForm: React.FC = () => {
  ...
  const [dishData, setDishData] = useState<FormDishType>({
    ...
    imageSrc:
      getImageNameFromURL(dishUpdateData?.image ? dishUpdateData?.image : "") ||
      "",
    image: dishUpdateData?.image || "",
    ingredients: dishUpdateData?.ingredient?.split(",") || [],
    label: dishUpdateData?.label || "",
    dietaryRestriction: dishUpdateData?.dietry || "",
    name: dishUpdateData?.name || "",
    price: dishUpdateData?.price || 0,
    restaurant: {
      name: dishUpdateData?.restaurantId?.name || "",
      id: dishUpdateData?.restaurantId?._id || "",
    },
    category: {
      name: dishUpdateData?.category?.name || "",
      id: dishUpdateData?.category?._id || "",
    },
    available: dishUpdateData?.available || false,
    recommended: dishUpdateData?.recommended || false,
    specialOffer: dishUpdateData?.specialOffer || false,
    specialOfferPrice: +(dishUpdateData?.specialOfferPrice ?? 0) || 0,
    additionalOption:
      (dishUpdateData?.additionalOption?.length ?? 0) > 0
        ? [...(dishUpdateData?.additionalOption ?? [])]
        : [
            {
              name: "",
              price: 0,
      ...
      >
        <div className="flex-1 h-full p-6 rounded font-montserrat">
          ...
          <div className="h-[calc(100vh-12rem)] overflow-y-auto pr-4 md:pr-0">
            <div className="grid items-center grid-cols-1 gap-4 py-4 lg:grid-cols-2">
              <input
                value={dishData.name}
                type="text"
              ...
              <textarea
                ...
                onChange={handleChange}
                name="description"
                className="w-full h-24 col-span-1 py-4 pl-4 font-medium border border-gray-400 rounded-md outline-none md:col-span-2 focus-within:border-blue-400"
