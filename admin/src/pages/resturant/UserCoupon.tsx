...
const UserCoupon = () => {
  ...
  const { isPending, isError, data, error, refetch } = useQuery<
    ...
    ApiError
  >({
    queryKey: [`coupon_owner/restaurantowneruser/coupon/${currentPage}`],
    queryFn: async () => {
      return await apiGetRequest<CouponeType>({
        url: `/restaurantowneruser/get/coupon/?page=${currentPage}&limit=${5}`,
      });
    },
  });
  ...
  const confirmhandler = () => {
    const deleteObj: UniDelet = {
      path: `/restaurantowneruser/${isDeletModal?.deletElementId}`,
    };
    ...
    rounded-md  font-philosopher max-w-full w-full shadow-md`}
        ...
        shadow-inner rounded-[0.26rem] outline-none `}
            ...
            <div className="relative flex items-center self-end ">
              ...
              >
                <Link to={"/coupons_owner/coupon_form"}>
