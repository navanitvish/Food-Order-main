...
import { useRestCategoriesPagination } from "../api/query";
import ResturantCategoryForm from "../forms/ResturantCategoryForm";
...
const ResturantCategory = () => {
  ...
  const { isPending, isError, data, error, refetch } =
    useRestCategoriesPagination(currentPage, 10);
  ...
  const confirmhandler = () => {
    const deleteObj: UniDelet = {
      path: `/restcategory/${isDeletModal?.deletElementId}`,
    };
  ...
  const navigatehandler = (id: string) => {
    navigate(`/resturant_category/${id}`);
  };

  return (
    <>
      {(isCategoryForm.creat || isCategoryForm.updateId) && (
        <ResturantCategoryForm
